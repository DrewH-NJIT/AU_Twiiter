...
class TwitterClient(context: Context) : OAuthBaseClient(
    ...
    null,
    "oauth://cprest" // this fixed so many fucking authorization problems. case-sensitive.
    // remember to add this to ur whitelist on twitter dev portal.
//    String.format(
...
//    )
    ...
    companion object {
        ...
        //        const val REST_URL = "https://api.twitter.com/2/" //new api level. look at their documentation for stuff.
        const val REST_URL = "https://api.twitter.com/1.1" //old api. used in terrible tutorial.
    ...
    fun getHomeTimeline(handler: JsonHttpResponseHandler) {
        val apiUrl =
//            getApiUrl("users/1572323460231184384")
            getApiUrl("statuses/home_timeline.json") // tutorial query
...
//
    fun loadMoreTweets(startID: Long, handler: JsonHttpResponseHandler) {
        val apiUrl =
//            getApiUrl("users/1572323460231184384")
            getApiUrl("statuses/home_timeline.json") // tutorial query
